...
from auth.models import StringsTable
...
strings_bp = Blueprint('strings', __name__, url_prefix='/api/strings')

@strings_bp.route('/stringsdata', methods=['POST'])
def get_strings_data():
    ...
    valid_columns = ["Project_Code", "Docket", "Strings", "Strings_Hits"]
    ...
    try:
        ...
        column_attr = getattr(StringsTable, column_name)
        
        query = db.session.query(StringsTable).filter(column_attr.like(f"%{keyword}%"))
        ...
        return jsonify({
            "count": count,  # <-- Added the count here
            "success": True,
            "message": "strings data retrieved successfully!",
            "data": data_list
        }), 200
        ...
        log_data = {
            'name':username,
            'tables_searched': 'Strings Search',
            'email': email,
            'date': start_date,
            'time': start_time,
